...
impl CompiledModuleViz {
    pub fn new(
        ...
        let internal_nodes: Vec<String> = module
            .internal_nodes(db)
            .iter()
            .map(|n| n.name(db).to_string())
            .collect();
...
impl InstructionViz {
    pub fn new(func: &Function, inst: mir::Inst, literals: &Rodeo) -> Self {
        ...
        let arguments: Vec<String> = if let InstructionData::PhiNode(phi) = data {
            func.dfg
                .phi_edges(phi)
                .map(|(block, val)| format!("block{}: {}", u32::from(block), format_value(func, val, literals)))
                .collect()
        } else {
            func.dfg
                .instr_args(inst)
                .iter()
                .map(|&v| format_value(func, v, literals))
                .collect()
        };
            ...
            .map(|ns| {
                ...
                let kind = match &ns.kind {
                    ...
                    NoiseSourceKind::NoiseTable { log, .. } => {
                        if *log { "table_log".to_string() } else { "table".to_string() }
                    }
                };
...
impl ParameterViz {
    pub fn new(kind: &ParamKind, val: mir::Value, db: &CompilationDB) -> Self {
        let (kind_str, name) = match kind {
            ...
            ParamKind::ParamGiven { param } => ("param_given", format!("${}_given", param.name(db))),
